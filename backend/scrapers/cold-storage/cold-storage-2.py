...
def get_url(category, page_number=1):
    print(f"https://coldstorage.com.sg/{category}/?Product_page={page_number}")
...
def grab_subcategories(category):
    first = requests.get(get_url(category))

    soup = BeautifulSoup(first.text, features="html.parser")
    time.sleep(1)
    return [a["href"] for a in soup.select(".dropdown-category a")]


c = [cat for cat in sorted(
    set(grab_subcategories('fruits'))) if cat.count("/") > 1 and not cat.startswith("http")]
...
with open("cs.json", "w") as f:
    ...
    for category in c:
        ...
        data[category] = scrape(category[1:])
    json.dump(data, f)
